...
static void w25q64_send_addr(w25q64_t *flash, uint32_t addr) {
...
void w25q64_example(void) {
    ...
    w25q64_t flash;
    w25q64_init(&flash, SPI1);
    uint8_t manuf; uint16_t dev;
    w25q64_read_id(&flash, &manuf, &dev);
    uint8_t buf[16];
    const uint8_t msg[16] = "Hello, Flash!\0";
    w25q64_sector_erase(&flash, 0u);
    w25q64_page_program(&flash, 0u, msg, sizeof(msg));
    w25q64_read(&flash, 0u, buf, sizeof(buf));
