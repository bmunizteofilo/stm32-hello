...
static void gd25q32_send_addr(gd25q32_t *flash, uint32_t addr) {
...
void gd25q32_example(void) {
    ...
    gd25q32_t flash;
    gd25q32_init(&flash, SPI2);
    uint8_t manuf; uint16_t dev;
    gd25q32_read_id(&flash, &manuf, &dev);
    uint8_t buf[16];
    const uint8_t msg[16] = "Hello, Flash!\0";
    gd25q32_sector_erase(&flash, 0u);
    gd25q32_page_program(&flash, 0u, msg, sizeof(msg));
    gd25q32_read(&flash, 0u, buf, sizeof(buf));
