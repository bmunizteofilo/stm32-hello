...
#define FT5406_REG_DATA 0x02

static ft5406_read_f ft_read;
static ft5406_write_f ft_write;
static I2C_TypeDef *ft_i2c;

#define FT5406_ADDR 0x38
#define I2C_TIMING_400KHZ 0x00300F33

static bool ft_hw_read(uint8_t reg, uint8_t *buf, uint8_t len) {
    return i2c_write_poll(ft_i2c, FT5406_ADDR, &reg, 1) &&
           i2c_read_poll(ft_i2c, FT5406_ADDR, buf, len);
}
...
static bool ft_hw_write(uint8_t reg, const uint8_t *buf, uint8_t len) {
    ...
    return i2c_write_poll(ft_i2c, FT5406_ADDR, tmp, (uint8_t)(len + 1));
}
...
bool ft5406_read(uint16_t *x, uint16_t *y) {
    ...
    uint8_t buf[5];
    if (!ft_read(FT5406_REG_DATA, buf, sizeof(buf))) return false;
...
bool ft5406_init_i2c(I2C_TypeDef *i2c) {
    ...
    i2c_enable(i2c, true);
    ft_i2c = i2c;
    return ft5406_init(ft_hw_read, ft_hw_write);
}

void ft5406_example(void) {
    uint16_t x, y;
    if (ft5406_init_i2c(I2C1) && ft5406_read(&x, &y)) {
        (void)x;
        (void)y;
    }
}
